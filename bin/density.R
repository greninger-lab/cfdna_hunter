...
df1 <- data.frame(PLOT_DATA='Human', V2=d1 )
df1 <- df1[sample(nrow(df1), 100000), ]
df2 <- data.frame(PLOT_DATA=srr_ref, V2=d2)
df <- rbind(df1, df2)
...
p <- ggplot(df, aes(x=V2, fill=!!PLOT_COL))+geom_density(alpha=.25)+scale_fill_manual(values=c("red","blue"))+
  ...
  geom_vline(data=mu, aes(xintercept=grp.median), color=c("red","blue"), linetype="dashed")
...
p <- ggplot(df, aes(x=V2, fill=!!PLOT_COL))+geom_density(alpha=.25)+scale_fill_manual(values=c("red","blue"))+
  ...
  scale_y_continuous(name = 'Fraction of Reads',limits=c(0,0.035))+
  theme_bw()+theme(panel.grid=element_blank())+
  geom_vline(aes(xintercept=mo1), color="red", linetype="dashed")+
  geom_vline(aes(xintercept=mo2), color="blue", linetype="dashed")
